...
/* Jacobi iteration */
    for (int step = 0; step < SMX; step++) {
        ...
        if (p%2 == 0){
            ...
            rc = MPI_Send(unew, sizeof(double), MPI_DOUBLE, pred, tag, MPI_COMM_WORLD);
            rc = MPI_Recv(u, sizeof(double), MPI_DOUBLE, pred, tag, MPI_COMM_WORLD, &status);
            rc = MPI_Send(unew + I - 1, sizeof(double), MPI_DOUBLE, succ, tag, MPI_COMM_WORLD);           
            rc = MPI_Recv(u + I + 1, sizeof(double), MPI_DOUBLE, succ, tag, MPI_COMM_WORLD, &status);
        } else{
            rc = MPI_Recv(u + I + 1, sizeof(double), MPI_DOUBLE, succ, tag, MPI_COMM_WORLD, &status);
            rc = MPI_Send(unew + I - 1, sizeof(double), MPI_DOUBLE, succ, tag, MPI_COMM_WORLD);
            rc = MPI_Recv(u, sizeof(double), MPI_DOUBLE, pred, tag, MPI_COMM_WORLD, &status);
